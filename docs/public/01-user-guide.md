...
SoloSpec v0.2 面向中文独立开发者，用一个入口 `$solo-spec` 把想法、迭代、Bug 修复或老项目接入，推进为统一目录下的项目文档、规格文档、TDD 任务、QA 证据和归档记录。
...
- 已安装专家 Skill 时，`$solo-spec` 也不能默认自动调用；必须先说明收益并等待用户确认。
...
.\scripts\install-project-skills.ps1 -ProjectPath <your-project> -Mode enhanced -Tool zcode
```

默认安装到 Codex 项目级目录 `.agents/skills/`。使用 zcode 时加 `-Tool zcode`，安装到 `.zcode/skills/`。

下文的 `<skills-root>` 指 `.agents/skills` 或 `.zcode/skills`。
...
专家增强只服务当前阶段。到达下列阶段的用户门禁前，`$solo-spec` 必须报告当前阶段专家增强状态，但不会枚举所有已安装 Skill。
...
- `$solo-spec` 必须在可增强阶段的门禁前报告当前阶段专家是否可用。
- 如果对应 SoloSpec 专家已安装，用户可以选择调用该专家，或跳过专家直接确认当前阶段。
- 如果对应 SoloSpec 专家未安装，`$solo-spec` 必须说明未检测到该专家，并允许用户跳过专家或指定其他 Skill / 工具审查当前阶段。
- 用户指定其他 Skill / 工具时，输出仍按外部 Reviewer / Advisor / Generator 处理。
